...
{
   if( !size_field ) size_field = sizeof(FieldType);
   memcpy(&field, source, size_field);
	source += size_field;
}

template <typename FieldType>
void fillField(std::optional<FieldType>& field, const char*& source, size_t size_field = 0)
{
   if( !size_field ) size_field = sizeof(FieldType);
   FieldType temp;
	memcpy(&temp, source, size_field);
	field = temp;
	source += size_field;
}
...
void fillField(FieldType* field, const char*& source, size_t size_field = 0)
{
	if (!size_field) size_field = strlen( source );
	memcpy(field, source, size_field);
	source += size_field;
}
